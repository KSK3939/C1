...
void ledRightShift(uint8_t num)
{
  ...
  for(uint8_t i = num; i > 0; i--)
  {
    HAL_GPIO_WritePin(led[i-1].port, led[i-1].number, led[i-1].offState);
    HAL_Delay(100);
  }
  HAL_Delay(500);
}
...
{
     HAL_GPIO_WritePin(led[*i].port, led[*i].number, led[*i].offState); // i를 끄면서
     *i = (*i+7) % 8; // i에 7을 더하고 8의 나머지를 구함(1을 빼는것과 같은 효과)
     HAL_GPIO_WritePin(led[*i].port, led[*i].number, led[*i].onState); // i를 켬
}

void ledRightShiftOne(uint8_t *i)
{
  HAL_GPIO_WritePin(led[*i].port, led[*i].number, led[*i].offState); // i를 끄면서
  *i = (*i+1) % 8; // i에 1을 더하고 8의 나머지를 구함
  HAL_GPIO_WritePin(led[*i].port, led[*i].number, led[*i].onState); // i를 켬
}
